...
export default function PostDetailPage() {
  ...
  // 1. Khai báo hàm loadPost trước (dùng useCallback để tối ưu)
  ...
  // 2. Gọi hàm trong useEffect sau khi đã khai báo
  useEffect(() => {
    loadPost();
  }, [loadPost]);
  ...
  async function handleComment(e) {
    ...
    try {
        ...
        setComment("");
        loadPost(); // Reload để hiện comment mới
    } catch(e) { console.error(e); }
  }
    ...
    <MainLayout>
      <main className="w-full lg:w-[600px] border-r border-slate-200/60 min-h-screen pb-20">
        ...
        <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 border-b border-slate-100 px-4 py-3 flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="p-2 hover:bg-slate-100 rounded-full"><ArrowLeft className="w-5 h-5 text-slate-700"/></button>
            ...
            {post.comments && post.comments.map(c => (
                <div key={c.id} className="p-5 hover:bg-slate-50 transition-colors">
                    <div className="flex gap-3">
                        <img src={getImageUrl(c.author_avatar) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${c.author_username || 'User'}`} className="w-10 h-10 rounded-full border border-slate-100 object-cover" alt="Commenter"/>
                        <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="font-bold text-slate-900 text-sm">{c.author_name}</span>
      ...
      {/* Right Sidebar - Mockup cho giống ảnh mẫu */}
