...
export default function MainLayout({ children }) {
  ...
  const getLinkClass = (path) => {
    // Logic active: Nếu đường dẫn hiện tại bắt đầu bằng path của link
        ...
        <aside className="hidden lg:flex w-72 flex-col fixed left-[max(0px,calc(50%-40rem))] top-0 h-screen border-r border-slate-200/60 bg-white/50 backdrop-blur-sm z-20 pt-6 px-6 pb-8">
          ...
          <nav className="flex-1 space-y-2">
            <Link to="/home" className={getLinkClass("/home")}>
                <Home className="w-7 h-7" /><span>Home</span>
            </Link>
            
            {/* ĐÃ SỬA LINK Ở ĐÂY (Thay # bằng đường dẫn thật) */}
            <Link to="/explore" className={getLinkClass("/explore")}>
                <Globe className="w-7 h-7" /><span>Global</span>
            </Link>
            ...
            <Link to="/messages" className={getLinkClass("/messages")}>
                <Mail className="w-7 h-7" /><span>Messages</span>
            </Link>
            
            <Link to={`/profile/${user?.username}`} className={getLinkClass(`/profile/${user?.username}`)}>
                <User className="w-7 h-7" /><span>Profile</span>
            </Link>
            
            {user?.is_admin && (
                <Link to="/admin" className={getLinkClass("/admin")}>
                    <Settings className="w-7 h-7" /><span>Admin</span>
                </Link>
            )}
          </nav>
          ...
          <div className="mt-auto pt-6 border-t border-slate-100">
            <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-100 transition-colors text-left group">
              <img 
                src={userAvatar} 
                className="w-10 h-10 rounded-full bg-indigo-50 border border-slate-200 object-cover" 
                alt="User"
              />
